...
public class QuestTalk2State extends AbstractState {
	...
	public QuestTalk2State(AlanWalker aw, float oldX, float oldY) {
		super(aw);
		sb = new SpriteBatch();
		...
		// Load Nurse
		npc = new Texture(Gdx.files.internal("resource/character/npc-quest-2.png"));
		...
		yesButton.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				if(loadPlayer.getProp().getProperty("Quest2").equals("start") && loadPlayer.getProp().getProperty("Quest2CountMonster").equals("10")) {
					aw.setScreen(new BattleState(aw, "JungleToCaveBoss", positionPlayerX, positionPlayerY));
				}
			}
		});
	...
	public void render(float delta) {
		...
		// Check Quest
		if(loadPlayer.getProp().getProperty("Quest2").equals("start")) {
			if(loadPlayer.getProp().getProperty("Quest2CountMonster").equals("10")) {
